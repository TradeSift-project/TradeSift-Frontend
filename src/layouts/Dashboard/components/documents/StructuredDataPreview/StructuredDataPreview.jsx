...
const StructuredDataPreview = ({ isOpen, onClose, documentName, documentType, fields = [], onExport }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/45 backdrop-blur-xs z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-[32px] border border-neutral-150 p-6 w-full max-w-2xl shadow-2xl relative flex flex-col max-h-[85vh]">
        ...
        {/* Data Table */}
        <div className="flex-1 overflow-y-auto border border-neutral-100 rounded-2xl mb-6">
          <table className="w-full border-collapse text-left text-xs text-gray-500 font-semibold">
            <thead>
              <tr className="border-b border-neutral-100 bg-neutral-50/50 text-[10px] font-bold uppercase tracking-wider text-gray-400">
            ...
            <tbody className="divide-y divide-neutral-100">
              {fields.map((field) => (
                <tr key={field.id} className="hover:bg-neutral-50/20">
                  <td className="px-5 py-3 text-[#0B0D12] font-semibold">{field.label}</td>
                  <td className="px-5 py-3 font-mono text-gray-700">{field.value || <span className="text-red-400 italic">Empty</span>}</td>
        ...
        <div className="border-t border-gray-100 pt-5 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <button
              ...
              className="flex-1 sm:flex-none inline-flex items-center justify-center gap-1.5 rounded-full border border-neutral-200 bg-white px-4 py-2.5 text-xs font-bold text-gray-700 transition hover:bg-neutral-50 uppercase tracking-wider"
            >
