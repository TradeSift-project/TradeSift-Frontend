...
const MappingActions = ({ onBack, onSave, onValidate, onExport, isReady }) => {
  return (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-8 pt-6 border-t border-gray-100 bg-white sticky bottom-0 p-4 shadow-[0_-4px_20px_rgba(0,0,0,0.02)] z-10 rounded-t-2xl">
      <button
        type="button"
        onClick={onBack}
        className="flex items-center gap-2 px-6 py-2.5 text-xs font-semibold text-gray-600 hover:text-gray-900 transition uppercase tracking-wider"
      >
        <ArrowLeft size={16} />
      ...
      <div className="flex items-center gap-3">
        <button
          type="button"
          onClick={onSave}
          className="flex items-center gap-2 px-6 py-2.5 rounded-full border border-gray-200 text-xs font-semibold text-gray-700 hover:bg-gray-50 transition uppercase tracking-wider"
        >
          <Save size={16} />
          Save Mapping
        </button>
